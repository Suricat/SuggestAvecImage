...
if(!empty($_POST)) {
	...
	$plxPlugin->setParam('isCatOnly', $_POST['isCatOnly'], 'numeric');
...
$var['isCatOnly'] = $plxPlugin->getParam('isCatOnly')=='' ?   0 : $plxPlugin->getParam('isCatOnly');
...
<form id="form_SuggestAvecImage" action="parametres_plugin.php?p=SuggestAvecImage" method="post">
	...
	<fieldset>
		...
		<?php plxUtils::printInput('title',$var['title'],'text','40-40') ?></p>
		...
		<p><label for="isCatOnly">
		<input type="checkbox" id="isCatOnly" name="isCatOnly" value="1"<?php echo ($var['isCatOnly']) ? ' checked' : '' ?> />
		<?php $plxPlugin->lang('L_ISCATONLY') ?></label></p>
